...
const these = {
   ...
   assign(classFile, config) {
      ...
      decoration.assign.forEach(key => {
         ...
         logger.debug('assign', decoration.assign, key, object);
         if (!object) {
            ...
            config[key] = Object.assign(config[key], object);
         }
      });
   },
   concat(classFile, config) {
      ...
      return these.readClassDecorationMaybe(classFile).then(decoration => {
         ...
         if (decoration.defaults) {
            ...
            if (decoration.concat) {
               these.concat(classFile, config);
            }
            if (decoration.assign) {
               these.assign(classFile, config);
            }
