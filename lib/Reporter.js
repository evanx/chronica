      ...
      const those = {
         async start() {
            if (config.helloDelay) {
               that.startTimeoutId = setTimeout(async () => {
                  alerter.sendAlert('RESTARTED', await those.getReport()).catch(err => {
                     logger.error('send hello:', err.stack);
                  });
               }, config.helloDelay);
            }
            if (config.dailyEnabled) {
               that.dailyIntervalId = setInterval(async () => {
                  var time = new Date();
                  if (time.getHours() === config.hour && time.getMinutes() === config.minute) {
                     logger.info('daily', time.getHours(), time.getMinutes(), config);
                     alerter.sendAlert('DAILY', await those.getReport()).catch(err => {
         ...
         async getReport() {
            ...
            lines.push(lodash.trim(peakLoad));
            lines.push(lodash.trim(rootDisk));
            return lines.join('\n');
         },
