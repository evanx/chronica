...
module.exports = {
   ...
   create(rootConfig, config) {
      ...
      assert(rootConfig.env.hostname, "" + rootConfig.toString());
      ...
      function formatSlackMessage(bot, subject, alertMessage) {
         let slackMessage =  'Chronica@' + rootConfig.env.hostname + ': *' + subject + '*';
      ...
      async function sendSlack(bot, subject, message) {
         ...
         let log = logger.method('sendSlack', [bot.name, subject, slackMessage]);
         try {
