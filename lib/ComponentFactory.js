...
export async function create(rootConfig) {
   ...
   async function init() {
      state.defaultConfig = ConfigDecorator.readFileSync(module.filename);
   ...
   async function initComponents() {
      ...
      state.componentNames.forEach(name => {
         ...
         ConfigDecorator.readFileSyncMaybe(classFile).then(decorator => {
            logger.info('decorator', name, decorator);
            Maybe.notEmpty(decorator.default).then(value => {
               config = Object.assign(config, decorator.default, value);
            }, err => {
               logger.warn('no decorator default:', name, err);
            });
            Maybe.notEmpty(decorator.asserts).then(asserts => {
               logger.warn('asserts', name, asserts);
            }, err => {
               logger.warn('no asserts', name, err);
            });
         }, err => {
            logger.warn('no decorator', name, err);
         });
