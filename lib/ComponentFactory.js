...
import EnviromentRegistry from './EnviromentRegistry';
...
export async function create(rootConfig) {
   ...
   async function createStores() {
      ...
      Object.keys(storeNames).forEach(name => {
         createStore(name);
      });
   }

   async function createStore(name) {
      logger.info('createStore', name);
      let storeClassFile = getClassFile('stores', name);
      let storeClass = require(storeClassFile);
      try {
         assert(!state.stores[name], 'unique store: ' + name);
         state.stores[name] = store;
         store = storeClass.create(rootConfig);
      } catch (err) {
         logger.error('createStore', name, err);
         throw err;
      }
   }
   ...
   async function initComponents() {
      ...
      state.componentNames.forEach(async (name) => {
         ...
         await startComponent(name, config, componentClassFile);
      });
   }
   ...
   function getClassFile(dir, name) {
      ...
      let classFile =  classModule + '/' + className;
