...
export async function create(rootConfig) {
   ...
   function createStore(name) {
      ...
      logger.info('createStore', name, storeClassFile);
      let storeClass = require(storeClassFile);
      try {
         assert(!state.stores[name], 'unique store: ' + name);
         let store = storeClass.create(rootConfig);
   ...
   async function startComponent(name, config, componentClassFile) {
      logger.info('startComponent', name, Object.keys(config));
      ...
      try {
         let component = componentClass.create(config, componentLogger, state);
