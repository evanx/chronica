...
export async function create(rootConfig) {
   ...
   async function init() {
      state.defaultConfig = YamlDecorator.decorateClass(module.filename, {});
   ...
   function getComponentDefaultConfig(name) {
      ...
      if (!defaultComponentConfig) { // TODO deprecate
         logger.info('empty defaultComponentConfig', name);
      } else if (defaultComponentConfig.defaults) {
         config = Object.assign(defaultComponentConfig.defaults, config);
      } else {
         logger.info('empty defaultComponentConfig', name);
      }
      return config;
